package sassy.girls.wallpaper.activity;
...
import sassy.girls.wallpaper.BuildConfig;
import sassy.girls.wallpaper.Config;
import sassy.girls.wallpaper.R;
import sassy.girls.wallpaper.adapter.AdapterTags;
import sassy.girls.wallpaper.adapter.AdapterWallpaperDetail;
import sassy.girls.wallpaper.callback.CallbackWallpaper;
...
import sassy.girls.wallpaper.rest.RestAdapter;
import sassy.girls.wallpaper.service.SetGIFAsWallpaperService;
import sassy.girls.wallpaper.service.SetMp4AsWallpaperService;
...
public class ActivityWallpaperDetail extends AppCompatActivity {
    ...
    AdsPref adsPref;
    RelativeLayout bgsharkTop;
    RelativeLayout bgsharkBottom;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adsManager.loadRewardedAd(adsPref.getIsRewardedPostDetails(), () -> {
            ...
            if (Constant.isRewarded) {
                switch (actionType) {
                    case Constant.SHARE:
                        showBottomSheetSetAction(wallpaper, Constant.SHARE);
                        break;
    ...
    private void initView() {
        ...
        bgsharkTop = findViewById(R.id.bg_shark_top);
        bgsharkBottom = findViewById(R.id.bg_shark_bottom);
        ...
        if (wallpaper.image_name != null) {
            ...
            if (wallpaper.rewarded == 1 && adsPref.getIsRewardedPostDetails()) {
                ...
                btnLock.setOnClickListener(view -> {
                    actionType = Constant.APPLY;
                    if (db.getRewarded(wallpaper.image_id) != null) {
                        showBottomSheetSetAction(wallpaper, Constant.APPLY);
                    } else {
                        showBottomSheetSetAction(wallpaper, Constant.REWARDED);
                    }
                });
                if (db.getRewarded(wallpaper.image_id) != null) {
    ...
    private void onPermissionGranted() {
        switch (actionType) {
            ...
            case Constant.DOWNLOAD:
                showBottomSheetSetAction(wallpaper, Constant.DOWNLOAD);
                break;
            case Constant.SET_WITH:
                showBottomSheetSetAction(wallpaper, Constant.SET_WITH);
                break;
            case Constant.SET_LIVE:
                showBottomSheetSetAction(wallpaper, Constant.SET_LIVE);
                break;
        }
    }
    ...
    private void showBottomSheetSetAction(Wallpaper wallpaper, String setAction) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            btnSetHomeScreen.setOnClickListener(v -> setAsWallpaper(view, wallpaperUrl, Constant.HOME_SCREEN, true, false));
            btnSetLockScreen.setOnClickListener(v -> setAsWallpaper(view, wallpaperUrl, Constant.LOCK_SCREEN, true, false));
        } else {
            btnSetHomeScreen.setVisibility(View.GONE);
            btnSetLockScreen.setVisibility(View.GONE);
        }

        btnSetBothScreen.setOnClickListener(v -> setAsWallpaper(view, wallpaperUrl, Constant.BOTH, true, false));
        btnSetWith.setOnClickListener(v -> {
            if (!verifyPermissions()) {
                actionType = Constant.SET_WITH;
                mBottomSheetDialog.dismiss();
                return;
            }
            setAsWallpaper(view, wallpaperUrl, Constant.SET_WITH, true, false);
        });
        btnSetCrop.setOnClickListener(v -> setAsWallpaper(view, wallpaperUrl, Constant.SET_CROP, true, false));
        btnSetLive.setOnClickListener(v -> {
            if (!verifyPermissions()) {
                actionType = Constant.SET_LIVE;
                mBottomSheetDialog.dismiss();
                return;
            }
            if (wallpaper.image_upload.contains(".gif") || wallpaper.image_url.contains(".gif")) {
                setAsWallpaper(view, wallpaperUrl, Constant.SET_GIF, true, false);
            } else if (wallpaper.image_upload.contains(".mp4") || wallpaper.image_url.contains(".mp4")) {
                setAsWallpaper(view, wallpaperUrl, Constant.SET_MP4, true, false);
            } else {
        ...
        btnSetSave.setOnClickListener(v -> {
            ...
            setAsWallpaper(view, wallpaperUrl, Constant.DOWNLOAD, true, false);
        });

        switch (setAction) {
            case Constant.SHARE:
                Tools.postDelayed(() -> setAsWallpaper(view, wallpaperUrl, Constant.SHARE, true, false), 10);
                break;
            case Constant.DOWNLOAD:
                Tools.postDelayed(() -> setAsWallpaper(view, wallpaperUrl, Constant.DOWNLOAD, true, false), 10);
                break;
            case Constant.SET_WITH:
                Tools.postDelayed(() -> setAsWallpaper(view, wallpaperUrl, Constant.SET_WITH, true, false), 10);
                break;
            case Constant.SET_LIVE:
                Tools.postDelayed(() -> {
                    if (wallpaper.image_upload.contains(".gif") || wallpaper.image_url.contains(".gif")) {
                ...
                break;
            case Constant.REWARDED:
                Tools.postDelayed(() -> setAsWallpaper(view, wallpaperUrl, Constant.SET_UNLOCK_REWARDED, false, true), 10);
                break;
        }
    ...
    private void setAsWallpaper(View view, String wallpaperUrl, String setAction, boolean showProgress, boolean showWatchAd) {
        ...
        switch (setAction) {
            ...
            case Constant.SET_UNLOCK_REWARDED:
                        ...
                        .listener(new RequestListener<File>() {
                            ...
                            public boolean onResourceReady(File resource, Object model, Target<File> target, DataSource dataSource, boolean isFirstResource) {
                                try {
                                    ...
                                    mBottomSheetDialog.dismiss();
                                    if (setAction.equals(Constant.DOWNLOAD)) {
